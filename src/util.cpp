...
const char * const BITCOIN_CONF_FILENAME = "gaon.conf";
const char * const BITCOIN_PID_FILENAME = "gaond.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "gaon" is a composite category enabling all Gaon-related debug output
            if(ptrCategory->count(string("gaon"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GaonCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GaonCore
    // Mac: ~/Library/Application Support/GaonCore
    // Unix: ~/.gaoncore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GaonCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GaonCore";
#else
    // Unix
    return pathRet / ".gaoncore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty gaon.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override gaon.conf
